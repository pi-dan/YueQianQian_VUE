    ...
    >
      ...
      <div style="margin-bottom:10px"   >
        <el-radio-group v-model="searchselect">
          ...
          <el-radio-button label="书签"></el-radio-button>
        </el-radio-group>
      </div>
      <el-autocomplete
        ...
        @select="handleSelect"
        @keyup.enter.native="startSearchWWW"
      >
...
export default {
  ...
  methods: {
    ...
    querySearch (queryString, callback) {
      ...
      if (this.searchselect === '书签') {
        if (this.searchText !== '') {
          clearTimeout(this.timeout)
          this.timeout = setTimeout(() => {
            api
              .SearchBookmark(this.searchText.trim())
              .then(results => {
    ...
    startSearchWWW () {
      ...
      switch (this.searchselect) {
        case '百度':
          window.open(
            'https://www.baidu.com/s?wd=' + this.searchText,
            '_blank'
          )
          break
        case '谷歌':
        ...
        case 'Github':
          window.open(
            'https://github.com/search?q=' + this.searchText,
            '_blank'
          )
          break
      }
